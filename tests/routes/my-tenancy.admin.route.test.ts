import { action, loader } from "../../app/routes/my-tenancy.admin";
...
const mockedGetCounts =
  ...
  >;
const mockedGetAdminFormOptions = getAdminFormOptions as jest.MockedFunction<
  typeof getAdminFormOptions
>;
...
describe("my-tenancy.admin route", () => {
  beforeEach(() => {
    jest.resetAllMocks();
  ...
  test("loader returns counts, form options and tabular entity data", async () => {
    ...
    mockedGetAdminFormOptions.mockResolvedValue({
      ...
      teachers: [{ value: "t1", label: "Jane" }],
      classrooms: [{ value: "c1", label: "Room A" }],
    });
  ...
  test("action delegates create intent to createAdminEntity", async () => {
    mockedCreateAdminEntity.mockResolvedValue({
      ok: true,
      entity: "teacher",
    ...
    expect(result).toEqual({ ok: true, entity: "teacher", message: "Teacher created." });
    expect(mockedCreateAdminEntity).toHaveBeenCalledTimes(1);
  });

  test("action delegates deleteEntity intent to deleteAdminEntityForTenancy", async () => {
